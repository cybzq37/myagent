...
```python
from myagent import SimpleAgent, MyAgent, Config

# 创建 Agent（默认启用会话持久化�?
config = Config(session_enabled=True)
agent = SimpleAgent("assistant", MyAgent(), config=config)
...
# 保存�? memory/sessions/my-session-name.json
```
...
for session in sessions:
    ...
    print(f"�?Tokens: {session['metadata'].get('total_tokens', 0)}")
    print(f"总步�? {session['metadata'].get('total_steps', 0)}")
    print("---")
```
...
config = Config(
    ...
    auto_save_interval=10  # �?10 条消息自动保�?
)

agent = SimpleAgent("assistant", llm, config=config)

# �?10 条消息自动保存到 session-auto.json
agent.run("长时间任�?)
```
...
try:
    ...
    # 用户�?Ctrl+C 时自动保存为 session-interrupted.json
    print("会话已自动保�?)
...
for s in sessions:
    print(f"{s['filename']}: {s['metadata']['total_steps']} �?)
```
...
config = Config(
    session_enabled=True,
    session_dir="my-custom-sessions"  # 自定义目�?
)
agent = SimpleAgent("assistant", llm, config=config)
```
...
- 恢复会话时，建议使用相同的配置和工具
- 如果环境变化，框架会给出警告
- 可以通过 `check_consistency=False` 跳过检�?

### 3. 敏感信息

- 会话文件可能包含敏感信息（API Key、密码等�?
- 建议不要提交到版本控制系�?
- 添加 `memory/sessions/` �?`.gitignore`

### 4. 自动保存性能

- 自动保存会增�?I/O 开销
- 建议根据实际需求调�?`auto_save_interval`
- 对于短对话，建议禁用自动保存
