...
> 提供标准的文件读写编辑能力，内置乐观锁机制，确保多进�?�?Agent 协作时的数据安全
...
# 4. Agent 自动使用文件工具
result = agent.run("读取 config.py，然后修�?API_KEY �?'new_key_123'")
```
...
- `path` (必需): 文件路径
- `content` (必需): 文件内容
- `file_mtime_ms` (可�?: 缓存�?mtime（用于冲突检测）
...
3. Edit("config.py", file_mtime_ms=123456)
   ├─ 检查当�?mtime (123789) vs 缓存 mtime (123456)
   ├─ 不一�?�?返回 CONFLICT 错误
   └─ Agent 看到冲突，重�?Read
```
...
print(f"缓存�?mtime: {response.data['file_mtime_ms']}")
...
if response.status.value == "error":
    print(f"�?冲突检测成�? {response.error_info['message']}")
    # 输出: 文件自上次读取后被修改。当�?mtime=XXX, 缓存 mtime=YYY
```

### 示例 3：批量编�?

```python
from myagent.tools.builtin import MultiEditTool
...
print(response.text)  # 成功执行 3 个替换操�?
```
...
- `working_dir`: 工作目录，默认等�?project_root
...
### Q1: 为什�?Edit 返回 "old_string 必须唯一匹配" 错误�?

**原因**：EditTool 要求 `old_string` 在文件中只出现一次，以确保替换的精确性�?

**解决方案**�?
```python
# 方案 1：使用更具体�?old_string
edit_tool.run({
    "path": "config.py",
    "old_string": 'API_KEY = "old_key"',  # 包含更多上下�?
    "new_string": 'API_KEY = "new_key"'
})

# 方案 2：使�?MultiEdit 指定多个替换
multiedit_tool.run({
    "path": "config.py",
    "edits": [
        {"old_string": "第一处的内容", "new_string": "新内�?"},
        {"old_string": "第二处的内容", "new_string": "新内�?"}
    ]
})
```
