"""MyAgent统一LLM接口 - 支持OpenAI、Anthropic、Gemini等多种接口"""
...
class MyAgent:
    ...
    def __init__(
        ...
        **kwargs
        ...
        # 验证必要参数
        if not self.model:
            raise MyAgentException("必须提供模型名称（model参数或LLM_MODEL_ID环境变量）")
        if not self.api_key:
            raise MyAgentException("必须提供API密钥（api_key参数或LLM_API_KEY环境变量）")
        if not self.base_url:
            raise MyAgentException("必须提供服务地址（base_url参数或LLM_BASE_URL环境变量）")
