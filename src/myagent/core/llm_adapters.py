...
class BaseLLMAdapter(ABC):
    ...
    def __init__(self, api_key: str, base_url: Optional[str], timeout: int, model: str):
        # 初始化适配器所需的连接参数与模型信息。
        # 这些参数由 MyAgent 根据 base_url 自动选择适配器后传入，
        # 具体数值通常来自 .env（LLM_API_KEY / LLM_BASE_URL / LLM_TIMEOUT / LLM_MODEL_ID）。
        self.api_key = api_key  # API 密钥，用于鉴权
        self.base_url = base_url  # 服务地址，None 表示使用官方默认地址
        self.timeout = timeout  # 请求超时时间（秒）
        self.model = model  # 默认调用的模型名称
        # 客户端实例采用懒加载：首次调用时才创建，避免初始化阶段产生不必要的连接开销。
        # _client 用于同步调用，_async_client 用于异步调用（arun / arun_stream 场景）。
        self._client = None  # 同步客户端（懒加载，由子类 create_client() 创建）
        self._async_client = None  # 异步客户端（懒加载，由子类 create_async_client() 创建）
    ...
    async def astream_invoke(self, messages: List[Dict], **kwargs) -> AsyncIterator[str]:
        ...
        适用于子类只实现了同步 stream_invoke、但调用方需要异步流的场景。
        子类若有原生异步 SDK（如 openai.AsyncOpenAI），应重写本方法以获得更好性能。
        """
        # asyncio.Queue 作为线程与事件循环之间的数据通道：
        #   工作线程 _stream_to_queue 往里塞 chunk，主事件循环 await 取出。
        # queue 本身是协程安全的，可跨线程使用（配合 run_coroutine_threadsafe）。
        queue = asyncio.Queue()
        loop = asyncio.get_event_loop()

        def _stream_to_queue():
            """工作线程函数：在独立线程中执行同步流式调用，把结果逐个投递回事件循环。

            为什么用 run_coroutine_threadsafe：
                queue.put 是协程，不能在普通线程里直接 await。
                run_coroutine_threadsafe 会把协程提交回主事件循环执行，
                实现线程 → 事件循环的安全投递。
            哨兵值约定：
                None  —— 正常结束
                Exception —— 把异常对象当数据投递，主循环取出后 re-raise
            """
            try:
                ...
                # 无论成功还是异常，最后都投递 None 作为结束哨兵
                asyncio.run_coroutine_threadsafe(queue.put(None), loop)

        # 把同步流式方法丢到默认线程池执行，不阻塞事件循环。
        # 注意：这里没有 await，fire-and-forget，实际消费在下面的 while 循环里进行。
        loop.run_in_executor(None, _stream_to_queue)

        # 消费端：从队列逐个取出 chunk 并 yield，把同步迭代转成异步生成器。
        # 三种取值：
        #   None        → 收到结束哨兵，正常退出
        #   Exception   → 工作线程抛过异常，re-raise 给调用方
        #   str         → 正常的流式文本片段，yield 出去
        while True:
...
class OpenAIAdapter(BaseLLMAdapter):
    ...
    def stream_invoke(self, messages: List[Dict], **kwargs) -> Iterator[str]:
        ...
        try:
            ...
            raise MyAgentException(f"OpenAI API流式调用失败: {str(e)}")
    ...
    async def astream_invoke(self, messages: List[Dict], **kwargs) -> AsyncIterator[str]:
        ...
        try:
            ...
            raise MyAgentException(f"OpenAI API异步流式调用失败: {str(e)}")
...
class AnthropicAdapter(BaseLLMAdapter):
    ...
    def invoke(self, messages: List[Dict], **kwargs) -> LLMResponse:
        ...
        try:
            ...
            raise MyAgentException(f"Anthropic API调用失败: {str(e)}")
    ...
    def stream_invoke(self, messages: List[Dict], **kwargs) -> Iterator[str]:
        ...
        try:
            ...
            raise MyAgentException(f"Anthropic API流式调用失败: {str(e)}")
    ...
    def invoke_with_tools(self, messages: List[Dict], tools: List[Dict], **kwargs) -> LLMToolResponse:
        ...
        try:
            ...
            raise MyAgentException(f"Anthropic工具调用失败: {str(e)}")
...
class GeminiAdapter(BaseLLMAdapter):
    ...
    def invoke(self, messages: List[Dict], **kwargs) -> LLMResponse:
        ...
        try:
            ...
            raise MyAgentException(f"Gemini API调用失败: {str(e)}")
    ...
    def stream_invoke(self, messages: List[Dict], **kwargs) -> Iterator[str]:
        ...
        try:
            ...
            raise MyAgentException(f"Gemini API流式调用失败: {str(e)}")
    ...
    def invoke_with_tools(self, messages: List[Dict], tools: List[Dict], **kwargs) -> LLMToolResponse:
        ...
        try:
            ...
            raise MyAgentException(f"Gemini工具调用失败: {str(e)}")
