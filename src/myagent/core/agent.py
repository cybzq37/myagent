...
# TYPE_CHECKING 在运行时为 False，类型检查器（mypy/Pyright/IDE）分析时为 True。
# 这里的导入只在类型检查阶段执行，运行时被跳过，用于避免循环依赖：
#   - tools/registry.py 和 observability/trace_logger.py 等模块反向依赖 Agent，
#     若在顶部直接 import，Python 会报 ImportError。
# 使用这些类型做注解时，需写成字符串形式（如 "ToolRegistry"），
# 或在文件顶部加 `from __future__ import annotations`。
if TYPE_CHECKING:
...
class Agent(ABC):
    ...
    def __init__(
        ...
        # 历史管理器：负责对话历史的追加、压缩与序列化。
        #   - min_retain_rounds: 至少保留最近几轮完整对话，不参与压缩
        #   - compression_threshold: Token 占用率达到该阈值时触发压缩
        #     （超出 context_window * compression_threshold 后，将早期对话摘要化）
        self.history_manager = HistoryManager(
            min_retain_rounds=self.config.min_retain_rounds,
            compression_threshold=self.config.compression_threshold
        )

        # 工具输出截断器：当工具返回内容过长时，按行数/字节上限截断，
        # 防止单次工具调用撑爆上下文窗口。完整输出会另存到 output_dir 指向的文件。
        #   - max_lines: 保留的最大行数
        #   - max_bytes: 保留的最大字节数
        #   - truncate_direction: 截断方向（head 保留开头 / tail 保留结尾 / head_tail 两端各留一部分）
        #   - output_dir: 完整输出的落盘目录，供 Agent 后续按需回看
